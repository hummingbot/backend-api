...
class DManV4ScriptConfig(BaseClientModel):
    ...
    exchange: str = Field(
        ...
        client_data=ClientFieldData(
            prompt_on_new=True,
            prompt=lambda mi: "Enter the name of the exchange where the bot will operate (e.g., binance_perpetual):",
        ),
    )
    ...
    leverage: int = Field(
        ...
        client_data=ClientFieldData(
            prompt_on_new=True,
            prompt=lambda mi: "Set the leverage to use for trading (e.g., 20 for 20x leverage):",
        ),
    )
    initial_auto_rebalance: bool = Field(
        False,
        client_data=ClientFieldData(
            prompt_on_new=False,
            prompt=lambda mi: "Enable initial auto rebalance (True/False):",
        ),
    )
    extra_inventory_pct: Decimal = Field(
        0.1,
        client_data=ClientFieldData(
            prompt_on_new=False,
            prompt=lambda mi: "Set the extra inventory percentage for rebalancing (e.g., 0.1 for 10%):",
        ),
    )
    asset_to_rebalance: str = Field(
        "USDT",
        client_data=ClientFieldData(
            prompt_on_new=False,
            prompt=lambda mi: "Enter the asset to use for rebalancing (e.g., USDT):",
        ),
    )
    rebalanced: bool = Field(
        False,
        client_data=ClientFieldData(
            prompt_on_new=False,
            prompt=lambda mi: "Set the initial state of rebalancing to complete (True/False):",
        ),
    )
    ...
    candles_exchange: str = Field(
        ...
        client_data=ClientFieldData(
            prompt_on_new=True,
            prompt=lambda mi: "Enter the exchange name to fetch candle data from (e.g., binance_perpetual):",
        ),
    )
    ...
    order_amount: Decimal = Field(
        ...
        client_data=ClientFieldData(
            prompt_on_new=True,
            prompt=lambda mi: "Enter the base order amount in quote asset (e.g., 6 USDT):",
        ),
    )
    amount_ratio_increase: Decimal = Field(
        1.5,
        client_data=ClientFieldData(
            prompt_on_new=True,
            prompt=lambda mi: "Set the ratio to increase the amount for each subsequent level (e.g., 1.5):",
        ),
    )
    n_levels: int = Field(
        5,
        client_data=ClientFieldData(
            prompt_on_new=True,
            prompt=lambda mi: "Specify the number of order levels (e.g., 5):",
        ),
    )
    top_order_start_spread: Decimal = Field(
        0.0002,
        client_data=ClientFieldData(
            prompt_on_new=False,
            prompt=lambda mi: "Set the spread for the top order (e.g., 0.0002 for 0.02%):",
        ),
    )
    start_spread: Decimal = Field(
        0.03,
        client_data=ClientFieldData(
            prompt_on_new=False,
            prompt=lambda mi: "Enter the starting spread for orders (e.g., 0.02 for 2%):",
        ),
    )
    spread_ratio_increase: Decimal = Field(
        2.0,
        client_data=ClientFieldData(
            prompt_on_new=False,
            prompt=lambda mi: "Define the ratio to increase the spread for each subsequent level (e.g., 2.0):",
        ),
    )

    top_order_refresh_time: int = Field(
        60,
        client_data=ClientFieldData(
            prompt_on_new=False,
            prompt=lambda mi: "Set the refresh time in seconds for the top order (e.g., 60 for 1 minute):",
        ),
    )
    order_refresh_time: int = Field(
        ...
        client_data=ClientFieldData(
            prompt_on_new=False,
            prompt=lambda mi: "Enter the refresh time in seconds for all other orders (e.g., 7200 for 2 hours):",
        ),
    )
    cooldown_time: int = Field(
        ...
        client_data=ClientFieldData(
            prompt_on_new=False,
            prompt=lambda mi: "Specify the cooldown time in seconds between order placements (e.g., 30):",
        ),
    )
    ...
    stop_loss: Decimal = Field(
        ...
        client_data=ClientFieldData(
            prompt_on_new=True,
            prompt=lambda mi: "Set the stop loss percentage (e.g., 0.2 for 20%):",
        ),
    )
    take_profit: Decimal = Field(
        ...
        client_data=ClientFieldData(
            prompt_on_new=False,
            prompt=lambda mi: "Enter the take profit percentage (e.g., 0.06 for 6%):",
        ),
    )
    ...
    global_trailing_stop_activation_price_delta: Decimal = Field(
        0.025,
        client_data=ClientFieldData(
            prompt_on_new=True,
            prompt=lambda mi: "Enter the activation price delta for the global trailing stop (e.g., 0.01 for 1%):",
        ),
    )
    global_trailing_stop_trailing_delta: Decimal = Field(
        0.005,
        client_data=ClientFieldData(
            prompt_on_new=True,
            prompt=lambda mi: "Set the trailing delta for the global trailing stop (e.g., 0.002 for 0.2%):",
        ),
    )
    ...
    dynamic_spread_factor: bool = Field(
        ...
        client_data=ClientFieldData(
            prompt_on_new=False,
            prompt=lambda mi: "Enable dynamic spread factor (True/False):",
        ),
    )
    dynamic_target_spread: bool = Field(
        ...
        client_data=ClientFieldData(
            prompt_on_new=False,
            prompt=lambda mi: "Activate dynamic target spread (True/False):",
        ),
    )
    smart_activation: bool = Field(
        ...
        client_data=ClientFieldData(
            prompt_on_new=False,
            prompt=lambda mi: "Enable smart activation for orders (True/False):",
        ),
    )
    activation_threshold: Decimal = Field(
        ...
        client_data=ClientFieldData(
            prompt_on_new=False,
            prompt=lambda mi: "Set the activation threshold (e.g., 0.001 for 0.1%):",
        ),
    )
    price_band: bool = Field(
        False,
        client_data=ClientFieldData(
            prompt_on_new=False,
            prompt=lambda mi: "Enable price band filtering (True/False):",
        ),
    )
    price_band_long_filter: Decimal = Field(
        0.8,
        client_data=ClientFieldData(
            prompt_on_new=False,
            prompt=lambda mi: "Set the long filter for price band (e.g., 0.8 for 80%):",
        ),
    )
    price_band_short_filter: Decimal = Field(
        0.8,
        client_data=ClientFieldData(
            prompt_on_new=False,
            prompt=lambda mi: "Specify the short filter for price band (e.g., 0.8 for 80%):",
        ),
    )
...
class DManV4MultiplePairs(ScriptStrategyBase):
    ...
    def __init__(
        self, connectors: Dict[str, ConnectorBase], config: DManV4ScriptConfig
    ):
        ...
        order_levels = order_level_builder.build_order_levels(
            amounts=Distributions.geometric(
                n_levels=self.config.n_levels,
                start=float(self.config.order_amount),
                ratio=float(self.config.amount_ratio_increase),
            ),
            spreads=[Decimal(self.config.top_order_start_spread)]
            + Distributions.geometric(
                n_levels=self.config.n_levels - 1,
                start=float(self.config.start_spread),
            ...
            order_refresh_time=[self.config.top_order_refresh_time]
            + [self.config.order_refresh_time] * (self.config.n_levels - 1),
            ...
            dman_config = DManV4Config(
                ...
                candles_config=[
                    CandlesConfig(
                        ...
                        max_records=self.config.bollinger_band_length + 100,
