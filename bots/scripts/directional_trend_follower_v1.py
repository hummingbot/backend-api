...
from hummingbot.smart_components.controllers.trend_follower_v1 import (
    TrendFollowerV1,
    TrendFollowerV1Config,
)
...
class DirectionalTradingTrendFollowerConfig(BaseClientModel):
    ...
    stop_loss: Decimal = Field(
        ...
        client_data=ClientFieldData(
            prompt_on_new=True,
            prompt=lambda mi: "Set the stop loss percentage (e.g., 0.01 for 1% loss):",
        ),
    )
    take_profit: Decimal = Field(
        ...
        client_data=ClientFieldData(
            prompt_on_new=True,
            prompt=lambda mi: "Enter the take profit percentage (e.g., 0.03 for 3% gain):",
        ),
    )
    time_limit: int = Field(
        ...
        client_data=ClientFieldData(
            prompt_on_new=True,
            prompt=lambda mi: "Set the time limit in seconds for the triple barrier (e.g., 21600 for 6 hours):",
        ),
    )
    trailing_stop_activation_price_delta: Decimal = Field(
        ...
        client_data=ClientFieldData(
            prompt_on_new=True,
            prompt=lambda mi: "Enter the activation price delta for the trailing stop (e.g., 0.008 for 0.8%):",
        ),
    )
    trailing_stop_trailing_delta: Decimal = Field(
        ...
        client_data=ClientFieldData(
            prompt_on_new=True,
            prompt=lambda mi: "Set the trailing delta for the trailing stop (e.g., 0.004 for 0.4%):",
        ),
    )
    open_order_type: str = Field(
        "MARKET",
        client_data=ClientFieldData(
            prompt_on_new=False,
            prompt=lambda mi: "Specify the type of order to open (e.g., MARKET or LIMIT):",
        ),
    )
    ...
    order_amount_usd: Decimal = Field(
        ...
        client_data=ClientFieldData(
            prompt_on_new=True,
            prompt=lambda mi: "Enter the order amount in USD (e.g., 15):",
        ),
    )
    spread_factor: Decimal = Field(
        0,
        client_data=ClientFieldData(
            prompt_on_new=False, prompt=lambda mi: "Set the spread factor (e.g., 0.5):"
        ),
    )
    order_refresh_time: int = Field(
        ...
        client_data=ClientFieldData(
            prompt_on_new=False,
            prompt=lambda mi: "Enter the refresh time in seconds for orders (e.g., 300 for 5 minutes):",
        ),
    )
    ...
    sma_fast: int = Field(
        ...
        client_data=ClientFieldData(
            prompt_on_new=True,
            prompt=lambda mi: "Enter the SMA fast length (range 10-150, e.g., 20):",
        ),
    )
    sma_slow: int = Field(
        100,
        ...
        client_data=ClientFieldData(
            prompt_on_new=True,
            prompt=lambda mi: "Set the SMA slow length (range 50-400, e.g., 100):",
        ),
    )
    bb_length: int = Field(
        100,
        ...
        client_data=ClientFieldData(
            prompt_on_new=True,
            prompt=lambda mi: "Enter the Bollinger Bands length (range 100-200, e.g., 100):",
        ),
    )
    bb_std: float = Field(
        2.0,
        ...
        client_data=ClientFieldData(
            prompt_on_new=True,
            prompt=lambda mi: "Set the standard deviation for the Bollinger Bands (range 2.0-3.0, e.g., 2.0):",
        ),
    )
    bb_threshold: float = Field(
        ...
        client_data=ClientFieldData(
            prompt_on_new=True,
            prompt=lambda mi: "Specify the threshold for the Bollinger Bands as a safety mechanism to don't enter in the market (range 0.1-0.5, e.g., 0.2):",
        ),
    )
...
class DirectionalTradingTrendFollower(ScriptStrategyBase):
    ...
    def __init__(
        ...
        config: DirectionalTradingTrendFollowerConfig,
    ):
        ...
        triple_barrier_conf = TripleBarrierConf(
            ...
            if config.open_order_type == "MARKET"
        ...
        for trading_pair in config.trading_pairs.split(","):
            ...
            self.executor_handlers[trading_pair] = DirectionalTradingExecutorHandler(
