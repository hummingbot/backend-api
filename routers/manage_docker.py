...
async def is_docker_running(wa: JWTWalletAuthDep):
    return {"is_docker_running": docker_manager.is_docker_running()}


@router.get("/available-images/{image_name}")
async def available_images(image_name: str, wa: JWTWalletAuthDep):
...
async def active_containers(wa: JWTWalletAuthDep):
    return docker_manager.get_active_containers()


@router.get("/exited-containers")
async def exited_containers(wa: JWTWalletAuthDep):
    return docker_manager.get_exited_containers()


@router.post("/clean-exited-containers")
async def clean_exited_containers(wa: JWTWalletAuthDep):
    return docker_manager.clean_exited_containers()


@router.post("/remove-container/{container_name}")
async def remove_container(container_name: str, wa: JWTWalletAuthDep, archive_locally: bool = True, s3_bucket: str = None):
...
async def stop_container(container_name: str, wa: JWTWalletAuthDep):
    return docker_manager.stop_container(container_name)


@router.post("/start-container/{container_name}")
async def start_container(container_name: str, wa: JWTWalletAuthDep):
    return docker_manager.start_container(container_name)


@router.post("/create-hummingbot-instance")
async def create_hummingbot_instance(config: HummingbotInstanceConfig, wa: JWTWalletAuthDep):
...
async def pull_image(image: ImageName, wa: JWTWalletAuthDep):
    try:
